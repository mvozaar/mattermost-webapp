...
import PostAttachmentOpenGraph, {
    getBestImageUrl,
...
describe('components/post_view/PostAttachmentOpenGraph', () => {
    const post = {
        ...
        create_at: 1,
        message: 'https://securCom.me',
        metadata: {
            images: {
                'http://securCom.me/OpenGraphImage.jpg': {
                    height: 100,
                    width: 100,
                },
            },
            ...
            images: [
                {
                    height: 100,
                    secure_url: '',
                    url: 'http://securCom.me/OpenGraphImage.jpg',
                    width: 100,
                },
            ],
...
describe('getBestImageUrl', () => {
    test('should return nothing with no OpenGraph metadata or dimensions', () => {
        ...
        expect(getBestImageUrl(openGraphData, imagesMetadata, false)).toEqual(
            null,
        );
    });
    ...
    test('should return secure_url if specified', () => {
        ...
        expect(getBestImageUrl(openGraphData, imagesMetadata, false)).toEqual(
            openGraphData.images[0].secure_url,
        );
    });
    ...
    test('should return a proxied image URL if the image proxy is enabled', () => {
        ...
        expect(
            getBestImageUrl(openGraphData, imagesMetadata, true).endsWith(
                `/api/v4/image?url=${encodeURIComponent(
                    openGraphData.images[0].url,
                )}`,
            ),
        ).toEqual(true);
    ...
    test('should not mangle a URL that is already proxied if the image proxy is enabled', () => {
        const openGraphData = {
            images: [
                {
                    url: `${Client4.getBaseRoute()}/image?url=${encodeURIComponent(
        ...
        expect(getBestImageUrl(openGraphData, imagesMetadata, true)).toEqual(
            openGraphData.images[0].url,
        );
    });
