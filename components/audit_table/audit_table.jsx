    ...
    for (var i = 0; i < props.audits.length; i++) {
        ...
        accessList[i] = (
            <tr key={audit.id}>
                ...
                {uContent}
                <td className={'word-break--all' + descStyle}>
                    {auditInfo.desc}
                </td>
                {iContent}
                {sContent}
...
export function formatAuditInfo(
    audit,
    formatMessage,
    currentUser,
    getByName,
        ...
        switch (actionURL) {
            ...
            case '/channels/create_direct':
                auditDesc = formatMessage(holders.establishedDM, {
                    username: getDirectTeammate(channelObj.id).username,
                });
            ...
            default: {
                ...
                if (channelInfo[1]) {
                    ...
                    if (userIdField.indexOf('user_id') >= 0) {
                        userId =
                            userIdField[userIdField.indexOf('user_id') + 1];
        ...
        switch (actionURL) {
            ...
            case '/oauth/allow':
                if (oauthInfo[0] === 'attempt') {
                    auditDesc = formatMessage(
                        holders.attemptedAllowOAuthAccess,
                    );
                } else if (oauthInfo[0] === 'success') {
                    auditDesc = formatMessage(holders.successfullOAuthAccess);
                } else if (
                    oauthInfo[0] ===
                    'fail - redirect_uri did not match registered callback'
                ) {
        ...
        switch (actionURL) {
            case '/hooks/incoming/create':
                if (webhookInfo === 'attempt') {
                    ...
                    auditDesc = formatMessage(
                        holders.succcessfullWebhookCreate,
                    );
        ...
        switch (actionURL) {
            case '/license/add':
                if (licenseInfo === 'attempt') {
                    ...
                    licenseInfo === 'failed - expired or non-started license'
                ) {
