...
describe('components/AddUserToChannelModal', () => {
    ...
    it('should match snapshot', () => {
        const wrapper = shallow(<AddUserToChannelModal {...baseProps} />);

        expect(
            wrapper.find('#add-user-to-channel-modal__add-button').props()
                .disabled,
        ).toBe(true);
        expect(
            wrapper.find('#add-user-to-channel-modal__user-is-member').exists(),
        ).toBe(false);
        expect(
            wrapper.find('#add-user-to-channel-modal__invite-error').exists(),
        ).toBe(false);
    ...
    it('should display error message if user is a member of the selected channel', () => {
        ...
        const wrapper = shallow(<AddUserToChannelModal {...props} />);
    ...
    describe('didSelectChannel', () => {
        it("should fetch the selected user's membership for the selected channel", () => {
            ...
            const selection = {
                channel: {id: 'someChannelId', display_name: 'channelName'},
            };

            wrapper.instance().didSelectChannel(selection);
            expect(props.actions.getChannelMember).toBeCalledWith(
                'someChannelId',
                'someUserId',
            );
        });
