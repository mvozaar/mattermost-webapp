...
import {
    getCurrentUserId,
    getUserByUsername as selectUserByUsername,
    ...
    return (dispatch, getState) => {
        ...
        if (path === 'channels') {
            if (identifier.length === LENGTH_OF_ID) {
                ...
                const moreChannelsByName = getOtherChannels(state).find(
                    (chan) => chan.name === identifier,
                );
            ...
            if (identifier.indexOf('@') === 0) {
                ...
                handleError(
                    match,
                    history,
                    getRedirectChannelNameForTeam(state, teamObj.id),
                );
            }
        }
    };
...
export function goToChannelByChannelName(match, history) {
    return async (dispatch, getState) => {
        ...
        if (!channel || !member) {
            const {data, error: joinError} = await dispatch(
                joinChannel(
                    getCurrentUserId(state),
                    teamObj.id,
                    null,
            ...
            if (joinError) {
                const {data: data2, error: getChannelError} = await dispatch(
                    getChannelByNameAndTeamName(team, channelName, true),
                );
...
function goToGroupChannelByGroupId(match, history) {
    return async (dispatch, getState) => {
        ...
        if (!channel) {
            const {data, error} = await dispatch(
                joinChannel(getCurrentUserId(state), teamObj.id, null, groupId),
            );
...
function handleChannelJoinError(match, history, defaultChannel) {
    ...
    history.push(
        team
            ? `/error?type=channel_not_found&returnTo=/${team}/channels/${defaultChannel}`
            : '/',
    );
}
