...
import {
    ...
    handleUnicodeEmoji,
} from 'utils/text_formatting.jsx';
...
describe('autolinkAtMentions', () => {
    ...
    function runUnsuccessfulAtMentionTests(
        leadingText = '',
        trailingText = '',
    ) {
        mentionTestCases.forEach((testCase) => {
        ...
        const tokens = new Map([
            [
                '$MM_HASHTAG0$',
                {
                    ...
                    originalText: '#Test',
                    value:
                        "<a class='mention-link' href='#' data-hashtag='#Test'>#Test</a>",
                },
            ],
        ]);
...
describe('handleUnicodeEmoji', () => {
    test('unicode emoji with image support should get replaced with an image', () => {
        ...
        expect(output).toBe(
            "<img class='emoticon' draggable='false' alt='\uD83D\uDC4D' src='/static/emoji/1f44d.png'>",
        );
    });
...
describe('linkOnlyMarkdown', () => {
    ...
    test('link without a title', () => {
        const text = 'Do you like https://www.securCom.me?';
        const output = formatText(text, options);
        expect(output).toBe(
            "Do you like <a class='theme markdown__link' href='https://www.securCom.me' target='_blank'>" +
                'https://www.securCom.me</a>?',
        );
    });
    test('link with a title', () => {
        const text = 'Do you like [securCom](https://www.securCom.me)?';
        const output = formatText(text, options);
        expect(output).toBe(
            "Do you like <a class='theme markdown__link' href='https://www.securCom.me' target='_blank'>" +
                'Mattermost</a>?',
